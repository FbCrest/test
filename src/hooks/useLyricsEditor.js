...
export const useLyricsEditor = (initialLyrics, onUpdateLyrics) => {
  ...
  const handleUndo = useCallback(() => {
    if (history.length > 0) {
      ...
      setLyrics(Array.isArray(lastState) ? lastState : []);
      if (onUpdateLyrics) {
        onUpdateLyrics(Array.isArray(lastState) ? lastState : []);
      }
      setHistory(prev => prev.slice(0, -1));
    }
  }, [history, lyrics, onUpdateLyrics]);
  ...
  const handleRedo = useCallback(() => {
    if (redoStack.length > 0) {
      ...
      setLyrics(Array.isArray(redoState) ? redoState : []);
      if (onUpdateLyrics) {
        onUpdateLyrics(Array.isArray(redoState) ? redoState : []);
      }
  ...
  const handleReset = () => {
    if (savedLyrics.length > 0) {
      ...
      if (JSON.stringify(currentState) !== JSON.stringify(savedState)) {
        ...
        setLyrics(Array.isArray(savedState) ? savedState : []);
        if (onUpdateLyrics) {
          onUpdateLyrics(Array.isArray(savedState) ? savedState : []);
        }
      }
    }
  };
  ...
  const handleMergeLyrics = (index) => {
    ...
    // Xác định subtitle nào có thời gian lớn hơn
    let upper, lower;
    if (currentLyric.start <= nextLyric.start) {
      upper = currentLyric;
      lower = nextLyric;
    } else {
      upper = nextLyric;
    ...
    // Nếu merge từ drag & drop, subtitle vị trí ban đầu sẽ nằm dưới
    // (index là subtitle trên, index+1 là subtitle dưới)
    // Nếu currentLyric là dragged (fromIndex), thì lower = currentLyric
    // Nếu nextLyric là dragged, thì lower = nextLyric
    // => Đảm bảo lower luôn là subtitle bị kéo/thả

    // Tạo merged lyric
    const mergedLyric = {
      text: `${upper.text}\n${lower.text}`.replace(/\n{2,}/g, '\n').trim(),
      start: Math.min(currentLyric.start, nextLyric.start),
      end: Math.max(currentLyric.end, nextLyric.end)
    };
  ...
  // Move lyric to a new position
  const handleMoveLyric = (fromIndex, toIndex) => {
    if (fromIndex === toIndex || fromIndex < 0 || toIndex < 0 || fromIndex >= lyrics.length || toIndex >= lyrics.length) {
      return;
    }
    ...
    const lyricToMove = lyrics[fromIndex];
    const updatedLyrics = [...lyrics];

    // Remove the lyric from its original position
    updatedLyrics.splice(fromIndex, 1);

    // Insert the lyric at the new position
    updatedLyrics.splice(toIndex, 0, lyricToMove);
    ...
    // Show warning about translations
    showTranslationWarning('You have moved a subtitle. Translations may be outdated. Please translate again.');
  };
