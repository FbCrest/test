...
const TimelineVisualization = ({
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (e.altKey && e.key === 's') {
        ...
        // [LOG_REMOVED] `Auto-scrolling ${disableAutoScroll.current ? 'disabled' : 'enabled'}`);
  ...
  useEffect(() => {
    ...
    if (!isScrollingRef.current &&
      ...
      // [LOG_REMOVED] `Auto-scroll triggered #${debugCounter.current}. Current time: ${currentTime}, Visible: ${visibleStart}-${visibleEnd}`);
      ...
      setTimeout(() => {
        isScrollingRef.current = false;
        // [LOG_REMOVED] 'Auto-scroll completed');
      }, 50);
    }
