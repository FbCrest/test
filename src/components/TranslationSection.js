...
const TranslationSection = ({ subtitles, videoTitle, onTranslationComplete }) => {
  ...
  const handleTranslate = async () => {
    ...
    try {
      ...
      // [LOG_REMOVED] 'Translation result:', result);
      // [LOG_REMOVED] 'Translation result type:', typeof result);
      // [LOG_REMOVED] 'Translation result length:', result ? result.length : 'null');
              ...
              <div className="translation-preview">
                ...
                <div className="translation-preview-content">
                  {translatedSubtitles.slice(0, 5).map((subtitle, index) => {
                    // Helper function to get formatted time display
                    const getTimeDisplay = (subtitle) => {
                      let startTime, endTime;
                      
                      // If this subtitle has an originalId, try to use timing from original subtitle
                      if (subtitle.originalId && subtitles) {
                        const originalSub = subtitles.find(s => s.id === subtitle.originalId);
                        if (originalSub) {
                          // Use original subtitle's timing
                          if (originalSub.start !== undefined && originalSub.end !== undefined) {
                            startTime = formatSecondsToSRTTime(originalSub.start);
                            endTime = formatSecondsToSRTTime(originalSub.end);
                          } else if (originalSub.startTime && originalSub.endTime) {
                            startTime = originalSub.startTime;
                            endTime = originalSub.endTime;
                          } else {
                            // Fallback to translated subtitle's timing
                            if (subtitle.start !== undefined && subtitle.end !== undefined) {
                              startTime = formatSecondsToSRTTime(subtitle.start);
                              endTime = formatSecondsToSRTTime(subtitle.end);
                        ...
                        // Use translated subtitle's own timing
                        if (subtitle.start !== undefined && subtitle.end !== undefined) {
                          startTime = formatSecondsToSRTTime(subtitle.start);
                          endTime = formatSecondsToSRTTime(subtitle.end);
                        } else if (subtitle.startTime && subtitle.endTime) {
                    ...
                    return (
                      <div key={index} className="preview-subtitle">
                        <span className="preview-time">{getTimeDisplay(subtitle)}</span>
