...
import ExtractAudioButton from './components/ExtractAudioButton';
...
import { abortAllRequests, PROMPT_PRESETS, forceRefreshPrompt } from './services/geminiService';
import { parseSrtContent } from './utils/srtParser';
import FileUploadInput from './components/inputs/FileUploadInput';

function PromptBadgeBar({ selectedPresetId, handleSelectPreset }) {
  return (
    <div className="prompt-badge-bar">
      {PROMPT_PRESETS.map((preset) => (
        <button
          key={preset.id}
          type="button"
          className={`prompt-badge${selectedPresetId === preset.id ? ' selected' : ''}`}
          onClick={e => { e.stopPropagation(); handleSelectPreset(preset.id); }}
          title={preset.purposeVi || preset.purpose || preset.title}
        >
          {preset.title}
        </button>
      ))}
...
function App() {
  ...
  const [handleSelectPreset, setHandleSelectPreset] = useState(null);
  ...
  const handleSrtUpload = async (srtContent, fileName) => {
    try {
      ...
      if (activeTab.includes('youtube') && selectedVideo) {
        try {
          ...
          try {
            ...
            console.error(t('errors.videoDownloadFailed', 'Video download failed'), error);
            // Reset downloading state
            setIsDownloading(false);
            setDownloadProgress(0);
            setStatus({
      ...
      console.error(t('errors.srtParsingFailed', 'Failed to parse SRT file'), error);
  ...
  const handleTranslationComplete = (translatedData) => {
    setTranslatedSubtitles(translatedData);
    if (translatedData && translatedData.length > 0) {
      setTargetLanguage(translatedData[0].language || '');
    }
  };

  // Auto refresh prompt with target language on app start
  useEffect(() => {
    forceRefreshPrompt();
  }, []);
            ...
            <div className="buttons-container">
              ...
              <ExtractAudioButton
                ...
                onAudioExtracted={(audioFile) => {
                  ...
                  setStatus({ 
                    message: `✅ Đã extract audio thành công! File giảm từ ${(uploadedFile.size / (1024 * 1024)).toFixed(1)}MB xuống ${(audioFile.size / (1024 * 1024)).toFixed(1)}MB`, 
                    type: 'success' 
                  });
                }}
                disabled={isGenerating || isDownloading}
