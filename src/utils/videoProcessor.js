...
import { getVideoDuration, getMaxSegmentDurationSeconds, getOptimalSegmentDuration } from './durationUtils';
...
export const processLongVideo = async (mediaFile, onStatusUpdate, t) => {
    ...
    try {
        ...
        const optimalSegmentDuration = getOptimalSegmentDuration(fileSizeMB, duration);
        
        // If media is shorter than the optimal segment duration, process it directly
        if (duration <= optimalSegmentDuration) {
            return await callGeminiApi(mediaFile, 'file-upload');
        }

        // Calculate number of segments using optimal duration
        const numSegments = Math.ceil(duration / optimalSegmentDuration);
        // console.log(`Splitting ${mediaType} into ${numSegments} segments with ${optimalSegmentDuration}s duration each`);

        // Initialize segment status array with pending status
        for (let i = 0; i < numSegments; i++) {
            // Calculate theoretical time range for initial display
            const startTime = i * optimalSegmentDuration;
            const endTime = Math.min((i + 1) * optimalSegmentDuration, duration);
        ...
        const splitResult = await splitVideoOnServer(
            mediaFile,
            optimalSegmentDuration,
            (progress, message) => {
                const overallProgress = 10 + Math.round(progress * 0.3); // 10% to 40%
                onStatusUpdate({
        ...
        if (isUsingStrongModel) {
            ...
            segments.forEach((segment, i) => {
                ...
                const startTime = segment.startTime !== undefined ? segment.startTime : i * optimalSegmentDuration;
                const segmentDuration = segment.duration !== undefined ? segment.duration : optimalSegmentDuration;
        ...
        const segmentPromises = segments.map(async (segment, i) => {
            ...
            const startTime = segment.startTime !== undefined ? segment.startTime : segmentIndex * optimalSegmentDuration;
            const segmentDuration = segment.duration !== undefined ? segment.duration : optimalSegmentDuration;
            const endTime = startTime + segmentDuration;
            ...
            if (segment.startTime !== undefined) {
                const theoreticalStart = segmentIndex * optimalSegmentDuration;
            ...
            try {
                ...
                // If not cached, process the segment with retry logic
                ...
                while (retryCount <= maxRetries) {
                    try {
                        ...
                        if ((segmentError.message.includes('500') || segmentError.message.includes('Server Error')) && retryCount <= maxRetries) {
                            console.warn(`Segment ${i+1} failed with 500 error, retrying (${retryCount}/${maxRetries})...`);
                            updateSegmentStatus(i, 'loading', t('output.retrying', 'Retrying... ({{retry}}/{{max}})', { retry: retryCount, max: maxRetries }), timeRange);
                        ...
                        // If not a 500 error or no retries left, throw the error
                        throw segmentError;
                    }
                }
            } catch (error) {
                ...
                const errorMessage = error.message.includes('500') 
                    ? t('output.serverError', 'Server error (500) - segment too large')
                    : error.message || t('output.processingFailed', 'Processing failed');
                updateSegmentStatus(i, 'error', errorMessage, timeRange);
