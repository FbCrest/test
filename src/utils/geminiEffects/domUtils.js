      ...
      if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
        ...
        mutation.addedNodes.forEach(node => {
          if (node.nodeType === Node.ELEMENT_NODE) {
            ...
            if (
              (node.classList &&
               (node.classList.contains('generate-btn') || node.classList.contains('retry-gemini-btn') || node.classList.contains('force-stop-btn') || node.classList.contains('extract-audio-button')))
            ) {
              shouldReinitialize = true;
            }

            // Check if the node contains buttons with the relevant classes
            const buttons = node.querySelectorAll('.generate-btn, .retry-gemini-btn, .force-stop-btn, .extract-audio-button');
            if (buttons.length > 0) {
              shouldReinitialize = true;
            }
          }
...
export const initializeButton = (button, initializedButtons, particles) => {
  ...
  // Create a collection of particles for this button - use more particles for better effect
  const initialCount = button.classList.contains('generate-btn') ? 15 : 8;
