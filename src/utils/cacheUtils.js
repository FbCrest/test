...
/**
 * Get subtitles from cache using the cache ID
 * @param {string} cacheId - The cache ID to look up
 * @returns {Promise<Object|null>} - Returns the cached subtitles or null if not found
 */
export const getSubtitlesFromCache = async (cacheId) => {
  try {
    if (!cacheId) return null;
    
    const cache = await caches.open('subtitles-cache');
    const response = await cache.match(`subtitles-${cacheId}`);
