  ...
  try {
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        case 413:
          errorMessage = `Lỗi: File quá lớn (${response.status}). Server không thể xử lý file này. Vui lòng nén hoặc chia nhỏ video xuống dưới 200MB.`;
          break;
        case 500:
          errorMessage = `Lỗi server (${response.status}): Không thể xử lý file. Có thể do file quá lớn hoặc định dạng không tương thích. Thử nén video hoặc chuyển đổi sang MP4.`;
          break;
        case 502:
        case 503:
        case 504:
          errorMessage = `Lỗi server tạm thời (${response.status}): Server đang quá tải. Vui lòng thử lại sau vài phút.`;
          break;
        case 400:
          errorMessage = `Lỗi định dạng file (${response.status}): File không hợp lệ hoặc bị hỏng. Vui lòng kiểm tra lại file.`;
          break;
        default:
          errorMessage = `Lỗi không xác định (${response.status}): Không thể tải lên ${mediaType}. Vui lòng thử lại.`;
      }
      ...
      try {
        ...
        try {
          ...
          if (textResponse && textResponse.length > 0) {
            errorMessage += ` Server trả về: ${textResponse.substring(0, 100)}...`;
          }
        } catch (textError) {
